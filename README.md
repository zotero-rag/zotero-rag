...
You can keep Zotero open during processing and author lookup. Enable **Settings > Advanced > Allow other applications on this computer to communicate with Zotero**. If Zotero locks its database, zqa immediately switches to its read-only local API on port 23119. This works offline and needs no Zotero API key. Base-relative linked files (`attachments:` paths using Zotero's Linked Attachment Base Directory) are skipped by both readers. Unavailable group libraries are skipped with a warning; retry processing once they are available. The running instance must use the selected data directory; zqa verifies this through a stored attachment's location. If the API is unavailable, or the library contains only linked files and its directory cannot be verified, close Zotero to use direct database access.
