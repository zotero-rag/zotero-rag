...
type CMap<'a> = HashMap<&'a str, &'a str>;
...
struct PdfParser<'a> {
    ...
    #[allow(dead_code)]
    font_type: HashMap<(PageID, &'a str), FontEncoding<'a>>,
}
...
impl<'a> Default for PdfParser<'a> {
    fn default() -> Self {
...
impl<'z> PdfParser<'z> {
    ...
    fn is_cid_keyed_font(
        ...
        page_id: PageID,
        font_key: &str,
    ) -> Result<FontEncoding<'_>, PdfError> {
        ...
        let font_encoding = str::from_utf8(font_encoding.unwrap().as_name().map_err(|_| {
        ...
        if ["/Identity-H", "/Identity-V"].contains(&font_encoding) || font_encoding == "/Type0" {
