    ...
    E: Write,
{
    ...
    // Status lines from the tool wrappers ([`crate::tools::mixins`]) and text segments from
    // the model both arrive over channels, and are pumped into the context's streams by the
    // select loop below. Neither producer can borrow the context directly: the wrappers and
    // the `on_text` callback must be `'static`, and the context stays with this function.
