...
use thiserror::Error;
use zqa_rag::llm::base::ApiClient;
...
use zqa_rag::{
    ...
    llm::{base::ChatRequest, errors::LLMError, factory::LLMClient, tools::Tool},
    reranking::common::{RerankProviderConfig, get_reranking_provider},
};

use crate::{common::UserDocument, utils::rag::ModelResponse};
                ...
                if matches!(
                    ...
                    futures.push(Box::pin(async move {
                        ...
                        let mut final_chunks = match query_method {
                            ...
                            _ => reranked_chunks.iter().map(|s| &**s).cloned().collect(),
                        };
                        ...
                        if matches!(query_method, QueryMethod::SubAgent) {
                            ...
                            let request = ChatRequest {
                                ...
                                message: get_prompt(
                                    &query.clone(),
                                    &final_chunks.iter().collect::<Vec<_>>(),
                                ),
                                tools: None,
                                on_tool_call: None,
                                on_text: None,
                            };
                            if let Ok(response) = self.client.send_message(&request).await {
